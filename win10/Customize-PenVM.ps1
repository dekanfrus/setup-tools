...
Function AdditionalTools {
	...
	Write-Host "Disabling Quick Access for Explorer"
	Set-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced' -Name Start_TrackDocs -Value 0
	Set-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer' -Name ShowRecent -Value 0
	
	if(Get-ItemProperty -Path 'HKCU:Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced' | Select-Object -ExpandProperty TaskbarGlomLevel) 
	{
	...
	{
		New-Item -Path 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced' -Name TaskbarGlomLevel -Force
		Set-ItemProperty -Path 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced' -Name TaskbarGlomLevel -Value 2
		Stop-Process -ProcessName explorer -Force
	}
	...
	Write-Host "Installing & Configuring Powerline for Cmder"
	cd $ScriptLocation
	...
	mkdir "C:\Logs\PowerShell"
	Copy-Item "$ScriptLocation\Microsoft.PowerShell_profile.ps1" $Profile.CurrentUserCurrentHost
	Copy-Item "$ScriptLocation\Microsoft.PowerShell_profile.ps1" $Profile.CurrentUserAllHosts

	. $Profile.CurrentUserCurrentHost
	. $Profile.CurrentUserAllHosts

	$PoshLocation = (Get-Module -Name 'oh-my-posh').path | Split-Path
	...
	if(Test-Path "C:\Tools\PowerShell\WindowsPowerShell\Modules"){
		...
		mkdir "C:\Tools\PowerShell\WindowsPowerShell\Modules" -Force
	}
	...
	cd "C:\Tools\PowerShell\WindowsPowerShell\Modules\LyncSniper"
	Import-Module LyncSniper.ps1
	cd "C:\Tools\PowerShell\WindowsPowerShell\Modules\MailSniper"
	Import-Module MailSniper.ps1
	cd "C:\Tools\PowerShell\WindowsPowerShell\Modules\EmailAddressMangler"
	Import-Module EmailAddressMangler.ps1
	cd "C:\Tools\PowerShell\WindowsPowerShell\Modules\Inveigh"
...
Function InstallChocolatey {
	...
	choco install mobaxterm --force --force-dependencies -y
	choco install foxitreader --force --force-dependencies -y
	choco install cmder --force --force-dependencies -y
	choco install slack --force --force-dependencies -y
	choco install lastpass --force --force-dependencies -y
	choco install bitwarden --force --force-dependencies -y
	choco install python3 --force --force-dependencies -y
	choco install openvpn --force --force-dependencies -y
	#choco install audacity --force --force-dependencies -y
	choco install libreoffice-fresh --force --force-dependencies -y
	...
	choco install ruby --force --force-dependencies -y
	choco install sysinternals --force --force-dependencies -y
	choco install googlechrome --force --force-dependencies -y
	choco install conemu --force --force-dependencies -y
	choco install cygwin --force --force-dependencies -y
	choco install winpcap --force --force-dependencies -y
	choco install wireshark --force --force-dependencies -y
	choco install greenshot --force --force-dependencies -y
	#choco install notepadplusplus.install --force --force-dependencies -y
	choco install git.install --force --force-dependencies -y
	choco install python2 --force --force-dependencies -y
	choco install nmap --force --force-dependencies -y
